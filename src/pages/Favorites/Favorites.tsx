...
const Favorites = ({darkMode}: IPageProps) => {
    ...
    return (
      <div className={darkMode ? "favorites-page-wrapper__dark" : "favorites-page-wrapper"}>
        ...
        {favorites.length ? 
        <div className={darkMode ? "fav-headline__wrapper__dark" : "fav-headline__wrapper" }>
          <div>
            <Link className="fav-goBack" to={routes.HOME}><ArrowLeft /></Link>
          </div>
          <div className="fav-headline">
            <h1>Favorite Translations</h1>
          </div>
        </div> : null}
        ...
        <div className={darkMode ? "fav-headline__wrapper__dark" : "fav-headline__wrapper" }>
          ...
          <div className="fav-headline">
            <h1>No favorite translations</h1>
          </div>
        </div> : null}
            ...
            <div key={index}>
              <div className="fav-translation">
                <div className="fav-translation__header">
                  <div>
                    <h1>
                      <span className={darkMode ? "fav-lang__dark" : "fav-lang"}>&#x2B50; {item.from}</span> &rarr; <span className={darkMode ? "fav-lang__dark" : "fav-lang"}>{item.to}</span>
                    </h1>
                  </div>
        ...
        <div className="fav-options">
          {favorites.length ? 
          <div className="clear-favs">
            <button className={darkMode ? "clear-favs-btn__dark" : "clear-favs-btn" } onClick={() => clearFavorites()}>Clear Favorites</button>
          </div> : null}
        </div>
      </div>
